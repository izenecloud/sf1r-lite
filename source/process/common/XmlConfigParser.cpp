        ...
        DistributedTopologyConfig& topologyConfig)
{
    if (topology)
    {
        ...
        // Current SF1 node
        ticpp::Element * cursf1node = getUniqChildElement(topology, "CurrentSf1rNode");
        ...
        parseMasterAgent(getUniqChildElement(cursf1node, "MasterServer", false), topologyConfig);
        parseWorkerAgent(getUniqChildElement(cursf1node, "WorkerServer", false), topologyConfig);
...
void SF1Config::parseMasterAgent(const ticpp::Element * master, DistributedTopologyConfig& topologyConfig)
{
    if (master)
    {
        ...
        getAttribute(master, "name", masterAgent.name_);
        getAttribute(master, "shardnum", masterAgent.shardNum_);
        ...
        for (collection_it = collection_it.begin(master); collection_it != collection_it.end(); collection_it++)
        {
            ...
            masterAgent.collectionList_.push_back(collection);

            bool distributive = false;
            getAttribute(collection_it.Get(), "distributive", distributive, false);
            if (distributive)
            {
                AggregatorUnit aggregatorUnit;
                getAttribute(collection_it.Get(), "name", aggregatorUnit.name_);
                downCase(aggregatorUnit.name_);
                masterAgent.addAggregatorConfig(aggregatorUnit);

                std::string shardids;
                getAttribute(collection_it.Get(), "shardids", shardids, false);
            }
        }
    }
}
...
void SF1Config::parseWorkerAgent(const ticpp::Element * worker, DistributedTopologyConfig& topologyConfig)
{
    if (worker)
    {
        ...
        for (collection_it = collection_it.begin(worker); collection_it != collection_it.end(); collection_it++)
        {
            ...
            getAttribute(collection_it.Get(), "name", serviceUnit.name_);
