...
bool ProductTokenizer::cmp_(const std::pair<std::string, float> x, const std::pair<std::string, float> y)
{
...
void ProductTokenizer::GetFeatureTerms(const std::string& src, uint32_t& brand, uint32_t& model, std::vector<uint32_t>& res, const size_t mode)
{
//read
...
*/
    if (!mode)
    {
        ...
        {
            res[i-2] = atoi(arr[i].c_str());
        } 
    }
    else
    {
        B5TTokenize(src, brand, model, res);
    }
}
