...
#include <boost/date_time/posix_time/posix_time.hpp>
    ...
    void  wikipediaGraph::save(std::ostream& os)
    {
...
/*
    void  wikipediaGraph::InitGraph() 
    {// 邻接表存储方式
    // example 1
       Node * d = new Node("iphone");
       //d->PrintNode();
       Node * a = new Node("苹果");
       Node * b = new Node("手机");
       Node * c = new Node("水果");
       Node * e = new Node("nokia");
       nodes_.push_back(e);
       nodes_.push_back(a);
       nodes_.push_back(b);
       nodes_.push_back(c);
       nodes_.push_back(d);
      // std::sort(nodes_.begin(),nodes_.end(),NodeCmpOperator);
        pr_.PrintPageRank(nodes_);
    // link in node
    // a <- b, c, d
       link2nodes(0,3);
    ...
    // b <- d
       ...
       c->InsertLinkdInNode(a);
       b->InsertLinkdInNode(d);
    // c <- b, d
       c->InsertLinkdInNode(a);
       e->InsertLinkdInNode(b);
...
*/   
    void  wikipediaGraph::link2nodes(const int& i,const int& j)
    {
    ...
    void  wikipediaGraph::GetTopics(const std::string& content, std::vector<std::string>& topic_list, size_t limit)
    {
        //cout<<"SetContentBias";
        ...
        {
             int i=(*sitr);
             if(nodes_[i]->GetAdvertiRelevancy()>0.1){pr.setPr(i,pr.getPr(i)+0.5);}
             if((pr.getPr(i)>TopicPR[limit-1].first&&nodes_[i]->GetInBoundNum()<10000)||pr.getContentRelevancy(i)*nodes_[i]->GetAdvertiRelevancy()>0.01)
             {
                for(int j=0;j<limit;j++)
                {
                   ...
                   if((pr.getPr(i)>TopicPR[j].first&&pr.getPr(i)>lowbound))
                   {
                      ...
                      if(pr.getPr(i)>0.8){lowbound=0.75;}
                      break;
                   }
                }
             }
        ...
        if(TopicPR[0].first<0.7){lowbound=0.4;}
        ...
        if(TopicPR[0].first>=0.6){lowbound=0.5;}
        if(TopicPR[0].first>0.71){lowbound=0.7;}
        if(TopicPR[0].first>0.8){lowbound=0.75;}
    ...
    void  wikipediaGraph::InitSubGaph(const int& index,set<int>& SubGraph,int itertime)
    {
    ...
    std::vector<pair<double,string> >  wikipediaGraph::SetContentBias(const string& content,PageRank& pr)
    {
       //cout<<"SetContentBiasBegin"<<endl;
       std::vector<pair<double,string> > ret;
       vector<pair<string,uint32_t> > RelativeWords=contentBias.getKeywordFreq(content);
      // cout<<"RelativeWords:";
       ...
       for(uint32_t i=0;i<RelativeWords.size();i++)
       {
          ...
          // cout<<RelativeWords[i].first<<" "<<RelativeWords[i].second<<"       ";
           ret.push_back(make_pair(log(double(advertiseBias.getCount(RelativeWords[i].first)+1.0))*0.25*RelativeWords[i].second +0.5,RelativeWords[i].first));
       }
       pr.InitMap();
       for(uint32_t i=0;i<RelativeWords.size();i++)
       {
    ...
    /*
    void  wikipediaGraph::pairBias(pair<string,uint32_t>& ReLativepair,bool reset)
    {
       bool HaveGet;
    ...
    */
    Node*  wikipediaGraph::getNode(const int&  id,bool& HaveGet)
    {
    ...
    int  wikipediaGraph::getIndex(const int&  id)
    {
    ...
    Node*  wikipediaGraph::getNode(const string&  str,bool& HaveGet)
    {
       int id=Title2Id(str);
      // cout<<title2id.size();
      // cout<<str<<"id"<<id<<"   "<<endl;
    ...
    void  wikipediaGraph::SetAdvertiseAll()
    {
    ...
    void  wikipediaGraph::SetAdvertiseBias(Node* node)
    {
        node->SetAdvertiRelevancy(advertiseBias.getCount(node->GetName()) );
    };
  /* */
    int  wikipediaGraph::Title2Id(const string& title)
    {         
    ...
    void  wikipediaGraph::CalPageRank(PageRank& pr)
    {
        pr.CalcAll(5);
        //pr_.PrintPageRank(nodes_);
    };
    void  wikipediaGraph::BuildMap()
    {
    ...
    void wikipediaGraph::simplifyTitle()
    {
...
void  wikipediaGraph::load(std::istream &f, int& id,string& name )
{
...
void  wikipediaGraph::loadAll(std::istream &f )
{
  int size;
...
void  wikipediaGraph::InitOutLink()
{
     for(unsigned i=0;i<nodes_.size();i++)
     {
...
string wikipediaGraph::ToSimplified(const string& name)
{
