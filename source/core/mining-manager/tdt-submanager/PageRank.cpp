...
void PageRank::SetContentRelevancy(int index,double contentRelevancy)
{
    ...
    {
      if(GetLinkin(index).contentRelevancy_<0.6)
        GetLinkin(index).contentRelevancy_+=0.3;
      else
        GetLinkin(index).contentRelevancy_=0.9;
    }
    else
    {
       
       if(contentRelevancy<1.0)
        GetLinkin(index).contentRelevancy_=min(contentRelevancy*contentRelevancy*GetLinkin(index).linkin_.size()*GetLinkin(index).outNumber_,0.2)+0.01;
       else
