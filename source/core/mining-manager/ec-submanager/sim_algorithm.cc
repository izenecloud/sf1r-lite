...
SimAlgorithm::SimAlgorithm(
        const std::string& working_dir, EcManager* ec_manager,
        uint32_t start_docid, double sim_threshold,
        idmlib::util::IDMAnalyzer* analyzer,
        idmlib::util::IDMAnalyzer* cma_analyzer,
        const std::string& kpe_resource_path)
    : working_dir_(working_dir), ec_manager_(ec_manager)
    , analyzer_(analyzer), cma_analyzer_(cma_analyzer)
    , kpe_resource_path_(kpe_resource_path)
...
void SimAlgorithm::AddTo_(uint32_t docid, const std::vector<GroupIdType>& gid_list)
{
    if (!gid_list.empty())//one doc exist
    {
        ...
        if (gid_list[0]) insert_to.resize(1, gid_list[0]);
        else
        {
