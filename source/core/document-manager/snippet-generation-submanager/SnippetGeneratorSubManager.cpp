...
bool SnippetGeneratorSubManager::getSnippet(const UString &text,
		const vector<uint32_t>& offsetPairs,
		const std::vector<UString>& queryTerms,
		const unsigned int maxSnippetLength, const bool bHighlight,
		izenelib::util::UString::EncodingType& encodingType, UString& snippetText) {	
	CREATE_SCOPED_PROFILER ( getSnippet, "SnippetGeneratorSubManager", "SnippetGeneratorSubManager::getSnippet");
	assert(NUM_OF_SENTENCES == 2);
	if (text.empty())
		return false;
	...
	if (max_score1 > 0) {
		if( text.isChineseChar(start_off1) )
			start_off1 = (start_off1-sen_off1 < maxSnippetLength/2)? sen_off1+1:start_off1 - maxSnippetLength/2;
	...
	if (max_score2 > 0) {
		if( text.isChineseChar(start_off2) )
			start_off2 = (start_off2-sen_off2 < maxSnippetLength/2)? sen_off2+1:start_off2-maxSnippetLength/2;
...
bool SnippetGeneratorSubManager::getSnippetText_(const UString& text,
		const std::vector<UString>& queryTerms, int snippetStarts,int nextSnippetStarts,
		const unsigned int maxSnippetLength, const bool bHighlight,
		izenelib::util::UString::EncodingType& encodingType, UString& snippetText) {
	if (text.empty())
		return false;	
...
double SnippetGeneratorSubManager::computeScore_(const unsigned int tf,
